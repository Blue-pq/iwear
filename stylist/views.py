...
class MyItems(ListView):
    ...
    def get_queryset(self):
        return super(MyItems, self).get_queryset().filter(
...
class MyForecast(TemplateView):
    """Retrieve today's forecast for user's location, display set of clothes
    from user's wardrobe.
    ...
    def get_template_names(self):
        return [{'GET': self.get_template_name,
                 'POST': self.post_template_name}[self.request.method]]
    ...
    def what_to_wear(self, low, high):
        ...
        tops = self.request.user.item_set.filter(
                item_type__in=Item.TOPS,
                temperature=high_classification)
        bottoms = self.request.user.item_set.filter(
                item_type__in=Item.BOTTOMS,
                temperature=high_classification)
        outers = []
        if high - low >= Item.DIURNAL_VARIATION or \
                high_classification == Item.COLD:
            outers = self.request.user.item_set.filter(
                    item_type__in=Item.OUTERS,
                    temperature=high_classification)
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.method == 'POST':
            ...
            context["high"] = high
            high_classification = Item.classify_temperature(high)
            context["sweater_needed"] = \
                high - low >= Item.DIURNAL_VARIATION or \
                high_classification == Item.COLD
